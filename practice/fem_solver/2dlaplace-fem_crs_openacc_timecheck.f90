...
contains
    ...
    subroutine cg_solver(n, values, col_idx, row_ptr, b, x)
        ...
        real(8) :: t_update_xr        ! x,r更新（累積）
        real(8) :: t_rr_new           ! rr_new計算（累積）
        ...
        do iter = 1,2 *n
            ...
            !$acc wait
            call system_clock(t1)
            t_update_xr = t_update_xr + real(t1 - t0) / real(count_rate)
            ...
            !$acc wait
            call system_clock(t1)
            t_rr_new = t_rr_new + real(t1 - t0) / real(count_rate)
        ...
        write(*,'(A,F12.8)') '[6] update x,r (cum)   [s]: ', t_update_xr
        write(*,'(A,F12.8)') '[7] rr_new (cum)       [s]: ', t_rr_new
        write(*,'(A,F12.8)') '[8] update p   (cum)   [s]: ', t_update_p
        write(*,'(A,F12.8)') '--- TOTAL              [s]: ', &
            t_matvec_init + t_init_rp + t_init_rr + &
            t_matvec_iter + t_pAp + t_update_xr + t_rr_new + t_update_p
        write(*,'(A,I10)')    '[0] iterations         [-]: ', iter
    end subroutine
